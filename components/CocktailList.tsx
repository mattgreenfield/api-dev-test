...
export const CocktailList: FC<CocktailListProps> = ({ heading, items, loading, error, addBlank = false, cols = 2 }) => {
            ...
            {items.map(({ strDrink, idDrink, strDrinkThumb }) => (
              <li key={idDrink + "_shopping"} className="relative">
                <Card isSquare={cols === 10}>
                  ...
                  <div className="absolute bottom-0 inset-x-0 bg-black font-bold text-white rounded-b-xl p-2 bg-opacity-70">{strDrink}</div>
                </Card>
              </li>
            ))}
            {!!emptySlots && (
              <button className="flex items-center justify-center border aspect-square rounded-2xl bg-gray-100 hover:bg-gray-50">
                Add Another
              </button>
            )}
